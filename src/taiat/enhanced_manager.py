"""
Enhanced TaiatManager with Global Optimized Prolog Path Planner Integration.

This module provides an enhanced version of TaiatManager that uses the global optimized
Prolog path planner to determine optimal execution sequences for Taiat queries.
"""

from functools import partial
from typing import Callable, Optional, List, Dict, Any
from taiat.base import TAIAT_TERMINAL_NODE, AgentData, TaiatQuery
...
class EnhancedTaiatManager:
    """
    Enhanced TaiatManager that uses global optimized Prolog path planning for optimal agent selection.
    ...
    def __init__(
        ...
        """
        Initialize the enhanced TaiatManager.
        ...
        if self.use_prolog_planning:
            try:
                ...
                test_result = plan_taiat_path_global_optimized(node_set, [])
    ...
    def _is_node_ready_to_run(self, node_name: str) -> bool:
        ...
        # Check if the node is not already running or done
        return self.output_status[node_name] == "pending"

    def _original_router_logic(self, node: str) -> str | None:
        """
        Original router logic from TaiatManager.

        This is the fallback routing logic when global optimized Prolog planning
        is not available or fails.

        Args:
            node: The current node that just completed

        Returns:
            Name of the next node to run, or None if done
        """
        if self.verbose:
    ...
    def get_execution_statistics(self) -> Dict[str, Any]:
        ...
        stats = {
            ...
            "prolog_planning_used": self.use_prolog_planning,
...
def create_enhanced_manager(
    ...
    **kwargs,
) -> EnhancedTaiatManager:
    """
    Create an enhanced TaiatManager with global optimized Prolog path planning.

    Args:
        node_set: Set of available agent nodes
        reverse_plan_edges: Reverse dependency edges
        desired_outputs: Desired outputs to plan for (optional)
        **kwargs: Additional arguments to pass to EnhancedTaiatManager

    Returns:
        Configured EnhancedTaiatManager instance
    """
    manager = EnhancedTaiatManager(
...
def validate_query_with_prolog(
    ...
    Args:
        node_set: Set of available agent nodes
        desired_outputs: Desired outputs to validate

    Returns:
        True if the query is valid, False otherwise
    """
    try:
        return plan_taiat_path_global_optimized(node_set, desired_outputs) is not None
    except Exception:
        return False
...
def plan_query_with_prolog(
    ...
    Args:
        node_set: Set of available agent nodes
        desired_outputs: Desired outputs to plan for

    Returns:
        List of node names in execution order, or None if planning failed
    """
    try:
        return plan_taiat_path_global_optimized(node_set, desired_outputs)
    except Exception:
        return None
