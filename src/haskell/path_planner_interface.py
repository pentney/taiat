...
class PathPlanner:
    ...
    def _find_haskell_binary(self) -> str:
        ...
        raise FileNotFoundError(f"Could not find or build Haskell binary: {binary_name}")
    ...
    def _build_haskell_binary(self, directory: Path) -> None:
        ...
        try:
            ...
            raise RuntimeError("Cabal not found. Please install Cabal to build the Haskell binary.")
    ...
    def plan_path(self, node_set: AgentGraphNodeSet, desired_outputs: List[AgentData], external_inputs: Optional[List[AgentData]] = None) -> List[str]:
        """
        ...
        if external_inputs is not None:
            input_data["externalInputs"] = [self._convert_agent_data_to_json(input_data) for input_data in external_inputs]
...
def plan_path(node_set: AgentGraphNodeSet, desired_outputs: List[AgentData], external_inputs: Optional[List[AgentData]] = None) -> List[str]:
    """
    Plan execution path using the Haskell implementation.
...
def validate_outputs(node_set: AgentGraphNodeSet, desired_outputs: List[AgentData]) -> bool:
    """
    Validate that all desired outputs can be produced.
